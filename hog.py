...
def roll_dice(num_rolls, dice=six_sided): 
    ...
    the outcomes nless any of the outcomes is 1. In that case, return 1u.
...
def piggy_points(score):
    ...
    # BEGIN PROBLEM 2
    if score // 10 == 0:
        return abs(0 - score) + 4
    elif score // 10 >= 10:
        return abs(score // 10 % 10 - score % 10) + 4
    else:
...
def more_boar(player_score, opponent_score):
    ...
    def smallest_number(num):
        ...
        while num > 0:
            last_digit = num % 10
            num = num // 10
            if last_digit < current_min:
                current_min = last_digit
        return current_min
    def largest_number(num):
        ...
        while num > 0:
            last_digit = num % 10
            num = num // 10
            if last_digit > current_max:
                current_max = last_digit
        return current_max
    if smallest_number(player_score) < smallest_number(opponent_score) and largest_number(player_score) > largest_number(opponent_score):
        return True
    else:
        return False
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            score0 = score0 + take_turn(strategy0(score0, score1), score1, dice, goal)# number of points we get
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        current_score = 0
        if who == 0:
            current_score = current_score + score0
        else:
            current_score = current_score + score1
        change = current_score - last_score
        if change > running_high:
            print('Player', who, 'has reached a new maximum point gain.', change, 'point(s)!')
        return announce_highest(who, current_score, max(change, running_high))
    return say
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    max_dice = 0
    while num_rolls <= 10:
        current_max = averaged_dice(num_rolls, dice)
